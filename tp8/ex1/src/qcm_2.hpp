...
{
public:
    Cat(std::vector<Human> o) // meilleure implémentation car la copie est faite avant (l'appelant aurait pu appeler avec un move)
        : owners { std::move(o) }
    {}
...
// plus on fait la copie avant, mieux c'est
...
public:
    void add_data(const std::string& key, const std::string& value)
    {
        ...
        values.emplace_back(new std::string { value }); // on ne fait pas de delete (et personne ne va le faire parce que c'est nous qui les créons et pas d'acces depuis l'exterieur)
    }
...
public:
    ...
    {
        for (auto a& : values)
            delete a.get(); // double déallocation
    }
...
#define ANSWER_22 false // mais c'est une très très bonne pratique
